# 1 "c:\\users\\shaftctx19\\desktop\\citrix\\plrcitrixica\\\\combined_PlrCitrixICA.c"
...
# 1 "c:\\users\\shaftctx19\\desktop\\citrix\\plrcitrixica\\\\combined_PlrCitrixICA.c" 2
...
# 2 "c:\\users\\shaftctx19\\desktop\\citrix\\plrcitrixica\\\\combined_PlrCitrixICA.c" 2
...
# 3 "c:\\users\\shaftctx19\\desktop\\citrix\\plrcitrixica\\\\combined_PlrCitrixICA.c" 2
...
# 4 "c:\\users\\shaftctx19\\desktop\\citrix\\plrcitrixica\\\\combined_PlrCitrixICA.c" 2
...
Action()
{
	...
	ctrx_sync_on_window("User login", ACTIVATE, 194, 175, 412, 248, "snapshot1", "last");
	lr_think_time(2);
	
	ctrx_type("admin", "", "last");
	lr_think_time(2);

	ctrx_key("TAB_KEY", 0, "", "last");
	lr_think_time(2);

	ctrx_type("Sh@ft123", "", "last");
	lr_think_time(2);
  ...
  plraju:
	...
	ctrx_sync_on_window("KWHotel Free (0.47.108) - [Calendar]", ACTIVATE, 0, 0, 801, 601, "snapshot8", "last");
	
	 
 x	= ctrx_sync_on_text_ocr(437, 62, 43, 22, "Meals", "NULL=snapshot_3", "last");
 lr_output_message("get text successs ...  %d",x);
	

	ctrx_get_text_ocr(0, 348, 67, 55, 18, "snapshot_2", text_BufferRaju, "last");
	lr_output_message("get text......... %s",text_BufferRaju);

	ctrx_get_text_ocr(0, 440, 66, 36, 19, "snapshot_1", text_bufferPlr, "last");
	lr_output_message("get text......... %s",text_bufferPlr);
	
	
	
	ctrx_get_window_name(window_plraju, "last");

if (strcmp(window_plraju, "KWHotel Free (0.47.108) - [Calendar]"))
{
	lr_output_message("Not Appear");
	lr_end_transaction("KWHotel Free", 1);
    goto plraju;
   	
} else{
    
    lr_output_message("Appear"); 
    lr_end_transaction("KWHotel Free", 0);
	...
	ctrx_sync_on_bitmap(437, 64, 44, 20, "e0873ec64ec2c7745e29524ca8bf4aae", "last");
	ctrx_get_bitmap_value(35, 120, 30, 50, bitmapBuf, "last" );
	...
	ctrx_sync_on_window("Warning", ACTIVATE, 190, 200, 421, 201, "snapshot20", "last");
	lr_think_time(2);
	...
	ctrx_sync_on_window("Update Application", ACTIVATE, 101, 88, 598, 423, "snapshot23", "last");
	lr_think_time(2);
...
# 5 "c:\\users\\shaftctx19\\desktop\\citrix\\plrcitrixica\\\\combined_PlrCitrixICA.c" 2
...
# 6 "c:\\users\\shaftctx19\\desktop\\citrix\\plrcitrixica\\\\combined_PlrCitrixICA.c" 2
